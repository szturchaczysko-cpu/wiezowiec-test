...
_unreadable_memory_ids = set()  # IDs z memory których nie można odczytać

# Whitelist znanych userów (typ=1) — mimo obecności underscore w nicku
# Sprzedawcy, telefoniści jako osoby, chatoszturek itp.
KNOWN_INDIVIDUAL_USERS = {
    "chatoszturek", "chatek", "chatosztur",
    # Sprzedawcy z underscore
    "kasia_k", "anna_m", "oliwia_m", "klaudia_k", "kinga_m",
    # Pojedyncze osoby z admina forum
    "sylwia", "EA", "JUSTYNA", "kb", "EwelinaG",
}
...
def _is_individual_user(nick):
    ...
    if nick.lower() in {u.lower() for u in KNOWN_INDIVIDUAL_USERS}:
        return True
    # Heurystyka: grupa = wszystko WIELKIMI lub z "/" (np. Operatorzy_UK/PL)
    # Underscore sam w sobie NIE decyduje (anna_m to user, TEAM_ATOMOWKI to grupa)
    if nick.isupper():
        return False
    if "/" in nick:
        return False
    # Grupy "mieszane": Telefoniści_DE, Operatorzy_DE — pierwsza litera duża + underscore + kod kraju
    import re as _re
    if _re.match(r'^[A-ZĄĆĘŁŃÓŚŹŻ][a-ząćęłńóśźż]+_[A-Z]{2,}$', nick):
        return False
    # Domyślnie: user (bezpieczniej)
    return True
...
def forum_write(post_id, do_odp_id, user_do, tresc, user_do_type=1, user_od=None, tytul=None, ai_user=None):
    if user_od is None:
        user_od = FORUM_USER
    if ai_user is None:
        ai_user = FORUM_USER
    
    # Typ autora: grupa (2) jeśli wygląda jak grupa, user (1) jeśli nick
    ...
    _flog(f"WRITE: user_od={user_od}, from_type={from_user_type}, tytul={tytul}")
...
def _extract_marker(text, start_match):
    """Extract full marker handling nested brackets in tresc."""
    ...
    while i < len(text) and depth > 0:
        ...
        i += 1
    if depth == 0:
        return text[start:i], start_match.group(1), text[start_match.end():i-1]
...
def parse_forum_markers(ai_response):
    ...
    for m in FORUM_MARKER_PATTERN.finditer(ai_response):
        raw, action_str, params_str = _extract_marker(ai_response, m)
        if not raw:
            continue
        ...
        marker = {"type": action, "raw": raw, "params": params}
...
def execute_forum_actions(ai_response, forum_memory=None, user_od=None, ai_user=None):
    ...
    forum_reads = []
    forum_writes = []
    _write_hashes = set()  # deduplication guard
    ...
    for marker in markers:
        if marker["type"] == "write":
            ...
            # Deduplication: skip duplicate writes
            _wh = hash(f"{cel}:{tresc[:60]}")
            if _wh in _write_hashes:
                _flog(f"DEDUP: pominięto duplikat WRITE cel={cel}")
                continue
            _write_hashes.add(_wh)
            ...
            if result.get("success"):
                ...
                # Oznacz w wyniku że trzeba fallback
...
_FORUM_THREADS_TEST = {
    "AUTOS_KURIERZY": {
        "post_id": 5687, "korzen_id": "DIRECT",
        "grupa": "TEAM_ATOMOWKI", "grupa_type": 2,
        "opis": "Zlecenie kuriera/etykiety/atomówki (§11.4)",
    },
    "SPEDYCJA_REKLAMACJE": {
        "post_id": 5693, "korzen_id": "DIRECT",
        "grupa": "SPEDYCJA_REKLAMACJE", "grupa_type": 2,
        "opis": "Problemy po zleceniu kuriera (§10.4)",
    },
    "CZATOSZTUR_REKLAMACJE": {
        "post_id": 5688, "korzen_id": "DIRECT",
        "grupa": "DZIAŁ_EKSPERCKI", "grupa_type": 2,
        "opis": "Reklamacja 'co dalej / można szturchać' (§5.3)",
    },
    "NIEPOZAMYKANE_AUSTAUSCHE": {
        "post_id": 5692, "korzen_id": "DIRECT",
        "grupa": "NIEPOZAMYKANE AUSTAUSCHE", "grupa_type": 2,
        "opis": "Niezamknięte Austausche / zielonka (§10.5)",
    },
    "CZATOSZTUR_DE": {
        "post_id": 5690, "korzen_id": "DIRECT",
        "grupa": "Operatorzy_DE", "grupa_type": 2,
        "opis": "DE — info dla sprzedawców, problemy ze skrzynią, FV, delegacje TEL, wpisy telefonistów o odbiorze",
    },
    "CZATOSZTUR_FR": {
        "post_id": 5689, "korzen_id": "DIRECT",
        "grupa": "Operatorzy_FR", "grupa_type": 2,
        "opis": "FR — info dla sprzedawców, problemy ze skrzynią, FV, delegacje TEL, wpisy telefonistów o odbiorze",
    },
    "CZATOSZTUR_UKPL": {
        "post_id": 5691, "korzen_id": "DIRECT",
        "grupa": "Operatorzy_UK/PL", "grupa_type": 2,
        "opis": "UK/PL — info dla sprzedawców, problemy ze skrzynią, FV, delegacje TEL, wpisy telefonistów o odbiorze",
    },
    "CZATOSZTUR_UK/PL": {
        "post_id": 5691, "korzen_id": "DIRECT",
        "grupa": "Operatorzy_UK/PL", "grupa_type": 2,
        "opis": "UK/PL (alias z ukośnikiem) — info dla sprzedawców, problemy ze skrzynią, FV, delegacje TEL",
    },
    "szturchacz_błędy": {
        "post_id": 5703, "korzen_id": "DIRECT",
        "grupa": "SZTURZE_WSPARCIE", "grupa_type": 2,
        "opis": "Raportowanie błędów/anomalii Szturchacza przez AI (§0.1.1.2)",
    },
}
...
def forum_write_to_thread(cel, tresc, user_do=None, do_odp_id=None, forum_memory=None, user_od=None, ai_user=None):
    ...
    tytul = None  # tytuł podwątku
    # Wyciągnij numer zamówienia z treści — używany jako tytuł zawsze
    ...
    if do_odp_id:
        ...
        mem_id = forum_memory[cel].get("id")
        if mem_id and mem_id in _unreadable_memory_ids:
            # ID jest w memory ale nie udało się go odczytać → nowy podwątek
            target_do_odp = None
            _flog(f"  DECYZJA: memory ID={mem_id} nieczytelne → NOWY PODWĄTEK zamiast podbijania")
        else:
            target_do_odp = mem_id
            _flog(f"  DECYZJA: kontynuacja z forum_memory, target={target_do_odp}")
    elif USE_NEW_SUBTHREADS:
        target_do_odp = None
        _flog(f"  DECYZJA: NOWY PODWĄTEK (USE_NEW=True, do_odp_id=None, tytul={tytul})")
    else:
...
def auto_load_forum_context(db, col_fn, numer_zamowienia, wsad_text=None):
    ...
    try:
        ...
        # --- AUTO-ODCZYT forum_id Z TAGU ---
        if not memory and wsad_text:
            forum_id_matches = re.findall(r'forum_id[=:](\d+)', wsad_text, re.IGNORECASE)
            if forum_id_matches:
                _flog(f"AUTO_LOAD: znaleziono forum_id w tagu: {forum_id_matches}")
                for fid_str in forum_id_matches:
                    fid = int(fid_str)
                    # Próbuj odczytać
                    result = forum_read_subtree(branch_id=fid, from_post_id=fid, nrzam=numer_zamowienia)
                    if result.get("success") and result.get("posts"):
                        # Udało się — zapisz do memory i kontynuuj normalnie
                        _flog(f"AUTO_LOAD: forum_id={fid} z tagu — odczyt OK, {result.get('count', 0)} postów")
                        cel_guess = "FORUM_TAG"
                        save_forum_memory(db, col_fn, str(numer_zamowienia), cel_guess, fid, f"auto z tagu forum_id={fid}")
                        memory = {cel_guess: {"id": fid, "co": f"wpis z tagu forum_id={fid}", "new_subthread": True}}
                        break
                    else:
                        _flog(f"AUTO_LOAD: forum_id={fid} z tagu — odczyt nieudany")
                
                # Jeśli żaden forum_id nie dał się odczytać — zapamiętaj info ale KONTYNUUJ
                if not memory:
                    ids_str = ", ".join(forum_id_matches)
                    _flog(f"AUTO_LOAD: forum_id z tagu ({ids_str}) nieczytelne — kontynuuję skanowanie")
                    _tag_fallback_msg = f"[FORUM_CONTEXT: TAG] (W tagu znaleziono forum_id={ids_str}, ale nie udało się odczytać wpisów. Operator powinien sprawdzić ręcznie na forum.)"
        # --- KONIEC AUTO-ODCZYT ---
        
        if not memory:
            # Zwróć info o nieczytelnych forum_id z tagu (jeśli były)
            _tag_fallback = locals().get('_tag_fallback_msg')
            if _tag_fallback:
                _flog(f"AUTO_LOAD: scan też pusty → zwracam info o forum_id z tagu")
        ...
        for cel, info in memory.items():
            ...
            if result.get("success") and result.get("posts"):
                ...
                _unreadable_memory_ids.add(forum_id)
        ...
        # Dodaj info o nieczytelnych forum_id z tagu (jeśli były)
        _tag_fallback = locals().get('_tag_fallback_msg')
        if _tag_fallback:
            context_parts.append(_tag_fallback)
