    ...
    for marker in markers:
        if marker["type"] == "write":
            ...
            if forum_id:
                ...
                # Brak pamięci — czytaj cały wątek (fallback)
...
# True = każdy case dostaje własny podwątek (docelowo)
# False = workaround, pisze pod stałym korzeniem
...
def auto_load_forum_context(db, col_fn, numer_zamowienia):
    ...
    for cel, info in memory.items():
        ...
        # Per-case flag: czy ten konkretny post był tworzony jako nowy podwątek?
        is_new_subthread = info.get("new_subthread", USE_NEW_SUBTHREADS)
        
        if is_new_subthread:
            # FORUM_ID jest korzeniem brancha → czytaj cały branch
            result = forum_read(branch_id=forum_id, max_pages=2)
        else:
            # Workaround — FORUM_ID jest postem wewnątrz brancha → filtruj subtree
            thread_info = FORUM_THREADS.get(cel)
            if thread_info and thread_info.get("korzen_id"):
        ...
        if result.get("success") and result.get("posts"):
            co = info.get("co", cel)
            posts = result["posts"][-10:]  # max 10 ostatnich
            
            # Sprawdź czy są odpowiedzi (nie tylko nasz post)
