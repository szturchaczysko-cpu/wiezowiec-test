...
if TEST_MODE:
    ...
    if not _prompts_doc.exists or not (_prompts_doc.to_dict() or {}).get("urls"):
        ...
        _urls["v4 forum weryfik"] = "https://raw.githubusercontent.com/szturchaczysko-cpu/szturchacz-test/refs/heads/main/v4_forum_weryfik.txt"
            ...
            for _fa_id, _fa_data in _fa_cases:
                ...
                with _fa_c3:
                    if st.button("🔓 Uwolnij", key=f"release_fa_{_fa_id}"):
                        db.collection(col("ew_cases")).document(_fa_id).update({
                            "autopilot_status": "calculated",
                        })
                        st.success(f"✅ {_fa_nrzam} uwolniony do kolejki!")
                        st.rerun()
            
            if st.button("🔓 Uwolnij WSZYSTKIE", key="release_all_fa"):
                for _fa_id, _ in _fa_cases:
                    db.collection(col("ew_cases")).document(_fa_id).update({
                        "autopilot_status": "calculated",
                    })
                st.success(f"✅ Uwolniono {len(_fa_cases)} casów!")
                st.rerun()
        else:
                    ...
                    if ai_response:
                        ...
                        if FORUM_ENABLED:
                            for forum_iter in range(3):  # max 3 iteracje forum
                                ...
                                forum_result = execute_forum_actions(ai_response, forum_memory=_fm_e3)
