    ...
    for cel, info in memory.items():
        ...
        if is_new_subthread:
            ...
            if thread_info and thread_info.get("korzen_id"):
                ...
                # Brak korzenia — czytaj cały wątek przez root i filtruj od forum_id
                post_id = thread_info.get("post_id") if thread_info else None
                if post_id:
                    _flog(f"  → root read: thread={post_id}, from={forum_id}")
                    result = forum_read(root_id=post_id, max_pages=3)
                    if result.get("success") and result.get("posts"):
                        # Filtruj od forum_id w dół (po Hierarchy)
                        start_hierarchy = None
                        for p in result["posts"]:
                            if p["Id"] == forum_id:
                                start_hierarchy = p["Hierarchy"]
                        ...
                        # else: post nie znaleziony w wątku — zwróć wszystko
                else:
