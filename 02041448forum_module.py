...
_FORUM_THREADS_TEST = {
    "AUTOS_KURIERZY": {
        "post_id": 5684, "korzen_id": 1488398,
    ...
    "KURIER_test": {
        "post_id": 5684, "korzen_id": 1488398,
        "grupa": "Sylwia", "grupa_type": 1,
        "opis": "Alias → AUTOS_KURIERZY (5684)",
    },
    "REKLA_test": {
        "post_id": 5616, "korzen_id": "DIRECT",
        "grupa": "Sylwia", "grupa_type": 1,
    ...
    "REKLAKURIER_TEST": {
        "post_id": 5682, "korzen_id": "DIRECT",
        "grupa": "Sylwia", "grupa_type": 1,
        "opis": "Test nowy wątek — kurier+reklamacja",
    },
}
...
def get_thread_info(cel):
    ...
    if not info:
        return None
    if info.get("korzen_id") is None and cel not in ["KURIER_test", "REKLA_test", "REKLAKURIER_TEST"]:
        discover_roots()
    return info
...
def auto_load_forum_context(db, col_fn, numer_zamowienia):
    ...
    try:
        ...
        for cel, info in memory.items():
            ...
            if is_new_subthread:
                result = forum_read_subtree(branch_id=forum_id, root_id=root_id, from_post_id=forum_id, nrzam=numer_zamowienia)
            else:
                if thread_info and thread_info.get("korzen_id") and thread_info.get("korzen_id") != "DIRECT":
