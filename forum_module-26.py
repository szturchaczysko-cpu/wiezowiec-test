...
def forum_read(branch_id=None, root_id=None, leaf_id=None, max_pages=5, login=None):
    """
    ...
    Args:
        ...
        login: nick użytkownika do odczytu (domyślnie FORUM_USER)
    """
    if login is None:
        login = FORUM_USER
    all_posts = []
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            # Debug: loguj odpowiedź HTTP przed raise_for_status
            if resp.status_code != 200:
                _flog(f"READ HTTP: status={resp.status_code}, url={resp.url}, body={resp.text[:300]}")
...
def forum_read_subtree(branch_id, from_post_id, login=None):
    """Czyta podwątek i filtruje posty od konkretnego wpisu w dół."""
    result = forum_read(branch_id=branch_id, login=login)
...
def discover_roots(login=None):
    ...
    for key, info in FORUM_THREADS.items():
        _flog(f"DISCOVER: {key} → root_id={info['post_id']}")
        result = forum_read(root_id=info["post_id"], max_pages=1, login=login)
        _flog(f"DISCOVER: {key} → success={result.get('success')}, count={result.get('count',0)}, error={result.get('error','none')}")
        if result["success"] and result["posts"]:
            for p in result["posts"]:
                if p["Do_Odpid"] == 0:
                    roots[key] = p["Id"]
                    info["korzen_id"] = p["Id"]
                    _flog(f"DISCOVER: {key} → korzen_id={p['Id']}")
                    break
            if key not in roots and result["posts"]:
                roots[key] = result["posts"][0]["Id"]
                info["korzen_id"] = result["posts"][0]["Id"]
                _flog(f"DISCOVER: {key} → korzen_id={result['posts'][0]['Id']} (fallback first post)")
    
    st.session_state["_forum_roots"] = roots
    return roots


def get_thread_info(cel, login=None):
    """Zwraca info o wątku. Odkrywa korzeń jeśli nieznany."""
    info = FORUM_THREADS.get(cel)
    if not info:
        return None
    if not info.get("korzen_id"):
        discover_roots(login=login)
    return info
...
def auto_load_forum_context(db, col_fn, numer_zamowienia, login=None):
    """Automatycznie odpytaj forum. Fallback: scan po numerze zamówienia."""
    _flog(f"AUTO_LOAD: start, nrzam={numer_zamowienia}, login={login or FORUM_USER}")
    ...
    if not memory:
        ...
        memory = _scan_forum_for_case(db, col_fn, str(numer_zamowienia), login=login)
    ...
    for cel, info in memory.items():
        ...
        if is_new_subthread:
            result = forum_read(branch_id=forum_id, max_pages=2, login=login)
        else:
            # get_thread_info wywołuje discover_roots() gdy korzen_id=None
            thread_info = get_thread_info(cel, login=login)
            if thread_info and thread_info.get("korzen_id"):
                ...
                # Brak korzenia nawet po discover_roots — fallback
                _flog(f"  → brak korzenia, próba leaf_id={forum_id}")
                result = forum_read(leaf_id=forum_id, max_pages=2, login=login)
                _flog(f"  → wynik fallback: success={result.get('success')}, count={result.get('count', 0)}, error={result.get('error', 'none')}")
...
def _scan_forum_for_case(db, col_fn, numer_zamowienia, login=None):
    ...
    for cel, info in FORUM_THREADS.items():
        ...
        try:
            result = forum_read(root_id=post_id, max_pages=3, login=login)
