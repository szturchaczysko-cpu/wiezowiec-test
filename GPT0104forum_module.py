...
def forum_write(post_id, do_odp_id, user_do, tresc, user_do_type=1, user_od=None, tytul=None):
    if user_od is None:
        user_od = FORUM_USER
...
def execute_forum_actions(ai_response, forum_memory=None, user_od=None):
    ...
    for marker in markers:
        if marker["type"] == "write":
            ...
            if forum_id and cel:
                result = read_forum_context_safely(cel=cel, forum_id=forum_id, nrzam=None, is_new_subthread=USE_NEW_SUBTHREADS, max_pages=3)
            elif forum_id:
                result = _raw_read_fallback(forum_id=forum_id, root_id=None, max_pages=3)
            elif cel and forum_memory and cel in forum_memory:
                mem_info = forum_memory[cel] or {}
                mem_id = mem_info.get("id")
                if mem_id:
                    result = read_forum_context_safely(
                        cel=cel,
                        forum_id=mem_id,
                        nrzam=None,
                        is_new_subthread=mem_info.get("new_subthread", USE_NEW_SUBTHREADS),
                        max_pages=3,
                    )
                else:
...
def _is_ai_post(post, forum_root_id=None):
    """Rozpoznaje nasze wpisy: korzeń z memory albo disclaimer w treści."""
    if not post:
        return False
    if forum_root_id is not None and post.get("Id") == forum_root_id:
        return True
    text = post.get("Text", "") or ""
    return "Jestem Chatoszturkiem AI" in text


def _raw_read_fallback(forum_id=None, root_id=None, max_pages=3):
    """Surowy fallback: branch → leaf+root → leaf."""
    if forum_id is None:
        return {"success": False, "error": "Brak forum_id"}

    tries = [
        ("raw branch", lambda: forum_read(branch_id=forum_id, root_id=root_id, max_pages=max_pages)),
    ]
    if root_id:
        tries.append(("raw leaf+root", lambda: forum_read(leaf_id=forum_id, root_id=root_id, max_pages=max_pages)))
    tries.append(("raw leaf", lambda: forum_read(leaf_id=forum_id, max_pages=max_pages)))

    last_error = "Brak danych"
    empty_result = None
    for label, fn in tries:
        _flog(f"  → próba odczytu: {label}")
        result = fn()
        if result.get("success") and result.get("posts"):
            return result
        if result.get("success"):
            empty_result = result
            last_error = "Brak postów"
        else:
            last_error = result.get("error", last_error)

    return empty_result or {"success": False, "error": last_error}


def read_forum_context_safely(cel, forum_id, nrzam=None, is_new_subthread=False, max_pages=3):
    """
    Ujednolicona, odporna ścieżka odczytu forum.
    Próbuje kilku wariantów GetPostTree, bo API bywa kapryśne zależnie od typu wpisu.
    """
    thread_info = FORUM_THREADS.get(cel, {}) if cel else {}
    root_id = thread_info.get("post_id")
    korzen_id = thread_info.get("korzen_id")

    tries = []
    added = set()

    def add_try(label, fn):
        if label not in added:
            tries.append((label, fn))
            added.add(label)

    if forum_id is None:
        return {"success": False, "error": f"Brak forum_id dla {cel or 'forum'}"}

    if not is_new_subthread and korzen_id and korzen_id != "DIRECT":
        add_try("subtree korzen", lambda: forum_read_subtree(branch_id=korzen_id, from_post_id=forum_id, nrzam=nrzam))

    add_try("subtree branch=self", lambda: forum_read_subtree(branch_id=forum_id, from_post_id=forum_id, nrzam=nrzam))

    if root_id:
        add_try("subtree leaf+root", lambda: forum_read_subtree(leaf_id=forum_id, root_id=root_id, from_post_id=forum_id, nrzam=nrzam))
        add_try("subtree root", lambda: forum_read_subtree(root_id=root_id, from_post_id=forum_id, nrzam=nrzam))
    ...
    raw_result = _raw_read_fallback(forum_id=forum_id, root_id=root_id, max_pages=max_pages)
    if raw_result.get("success") and raw_result.get("posts"):
        return raw_result

    if empty_result:
        return empty_result
    return {"success": False, "error": last_error or raw_result.get("error", "Brak danych") }
...
USE_NEW_SUBTHREADS = True  # zostawiamy nowe podwątki, ale z bezpieczniejszym fallbackiem odczytu
...
def forum_write_to_thread(cel, tresc, user_do=None, do_odp_id=None, forum_memory=None, user_od=None):
    ...
    tytul = None  # tytuł podwątku (tylko przy tworzeniu nowego)
    ...
    if do_odp_id:
        ...
        target_do_odp = None  # None = nowy podwątek (branch)
        # Tytuł = numer zamówienia z treści
        _nrzam_match = re.search(r'Zamówienie:\s*(\d{5,10})', tresc, re.IGNORECASE)
    ...
    try:
        ...
        for cel, info in memory.items():
            ...
            result = read_forum_context_safely(
                cel=cel,
                forum_id=forum_id,
                nrzam=numer_zamowienia,
                is_new_subthread=is_new_subthread,
                max_pages=2,
            )
            ...
            if result.get("success") and result.get("posts"):
                ...
                human_replies = [p for p in posts if not _is_ai_post(p, forum_root_id=forum_id)]
