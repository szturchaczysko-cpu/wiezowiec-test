...
def save_forum_memory(db, col_fn, numer_zamowienia, cel, forum_id, co=""):
    ...
    try:
        # update() z dot-notation tworzy zagnieżdżoną strukturę
        doc_ref.update({f"forum_posts.{cel}": entry})
        _flog(f"  → ZAPISANO (update)")
    except Exception:
        # Dokument nie istnieje — utwórz z proper nested dict
...
def auto_load_forum_context(db, col_fn, numer_zamowienia):
    ...
    # Fallback: memory istnieje ale forum_read zwrócił puste (timeout, brak odpowiedzi)
    # Wstrzykujemy minimalny kontekst żeby AI wiedziało że post już był i NIE pisało ponownie
    if memory:
        fallback = []
        for cel, info in memory.items():
            fid = info.get("id")
            ...
            co = info.get("co", cel)
            if fid:
                _flog(f"AUTO_LOAD: fallback kontekst dla {cel}, forum_id={fid}")
                fallback.append(
                    f"[FORUM_CONTEXT: {cel}] (wpis z {data}, id={fid}, brak nowych odpowiedzi — "
