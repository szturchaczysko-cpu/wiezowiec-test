...
def execute_forum_actions(ai_response):
    """
    Parsuje markery → wykonuje API calls → zwraca podmienioną odpowiedź.
    
    FORUM_WRITE → wyślij post → zamień marker na link + FORUM_ID
    FORUM_READ → pobierz posty → zwróć jako kontekst do wstrzyknięcia
    """
    ...
    for marker in markers:
        if marker["type"] == "write":
            ...
            if forum_id:
                ...
                # Czytaj cały wątek po nazwie
                info = get_thread_info(cel)
                if info:
                    result = forum_read(root_id=info["post_id"], max_pages=3)
                else:
                    result = {"success": False, "error": f"Nieznany cel: {cel}"}
            else:
...
def forum_write_to_thread(cel, tresc, user_do=None, do_odp_id=None):
    """Pisze na forum do wątku po nazwie celu (np. 'AUTOS_KURIERZY').
    FORUM_ID = Id nowego postu (do kontynuacji)."""
    info = get_thread_info(cel)
    if not info:
        return {"success": False, "error": f"Nieznany cel: {cel}"}
    
    target_do_odp = do_odp_id or info.get("korzen_id")
...
def auto_load_forum_context(db, col_fn, numer_zamowienia):
    ...
    for cel, info in memory.items():
        ...
        if result.get("success") and result.get("posts"):
            ...
            for p in result["posts"][-10:]:  # max 10 ostatnich postów per wątek
                date_str = p['DateAdd'][:10] if p.get('DateAdd') else '?'
                context_parts.append(
                    f"  [{date_str}] {p['UserAddName']} → {p['UserToName']}: "
