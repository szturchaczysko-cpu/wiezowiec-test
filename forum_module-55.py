    ...
    try:
        ...
        if not memory and wsad_text:
            ...
            if forum_id_matches:
                ...
                # Jeśli żaden forum_id nie dał się odczytać — info dla operatora
                if not memory:
                    ids_str = ", ".join(forum_id_matches)
                    _flog(f"AUTO_LOAD: forum_id z tagu ({ids_str}) nieczytelne — operator musi sprawdzić ręcznie")
                    return f"[FORUM_CONTEXT: TAG] (W tagu znaleziono forum_id={ids_str}, ale nie udało się odczytać wpisów. Operator powinien sprawdzić ręcznie na forum.)"
        # --- KONIEC AUTO-ODCZYT ---
