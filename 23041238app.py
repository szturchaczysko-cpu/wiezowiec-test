...
if TEST_MODE:
    ...
    _ALL_OPS = ["Emilia", "Oliwia", "Magda", "Ewelina", "Iwona", "Marlena", "Sylwia", "EwelinaG", "Andrzej", "Marta", "Klaudia", "kasia_k", "kinga", "Anna_m", "Romana"]
...
with tab_generuj:
    ...
    ALL_OPERATORS_LIST = ["Emilia", "Oliwia", "Magda", "Ewelina", "Iwona", "Marlena", "Sylwia", "EwelinaG", "Andrzej", "Marta", "Klaudia", "kasia_k", "kinga", "Anna_m", "Romana"]
...
with tab_autopilot:
    ...
    ALL_OPERATORS_LIST_DL = ["Emilia", "Oliwia", "Magda", "Ewelina", "Iwona", "Marlena", "Sylwia", "EwelinaG", "Andrzej", "Marta", "Klaudia", "kasia_k", "kinga", "Anna_m", "Romana"]
            ...
            if processed >= total:
                ...
                if case_info is None:
                    ...
                    _AP_TEL_JEZYKI = {
                        ...
                        "Oliwia": ["EN"], "Marta": ["PL"], "kasia_k": ["FR"], "kinga": ["DE"], "Anna_m": ["IT"],
                    }
...
with tab_cases:
    ...
    if not filtered:
        ...
        for doc_id, c in filtered[start:end]:
            ...
            swiezak_mark = "✨" if c.get("processed_via") == "swiezak" else ""
            idx_label = f" | 📦 {c.get('index_handlowy')}" if c.get('index_handlowy') else ""
            cc1, cc2 = st.columns([4, 1])
            with cc1:
                st.markdown(f"{si} {ap_mark}{swiezak_mark} **{c.get('numer_zamowienia', '?')}** — "
            ...
            # Sygnatura prompta dla świeżaków i manual
            if c.get("processed_prompt") and c.get("status") == "zakonczony":
                _pp = c.get("processed_prompt", "?")
                _pf = c.get("processed_prompt_filename", "")
                _via = c.get("processed_via", "manual")
                _via_label = "✨ Świeżak" if _via == "swiezak" else "📝 Ręczne"
                _ghl = c.get("processed_prompt_github_link", "")
                if _ghl:
                    st.caption(f"{_via_label} — prompt: **{_pp}** (`{_pf}`) · [GitHub]({_ghl})")
                else:
                    st.caption(f"{_via_label} — prompt: **{_pp}** (`{_pf}`)")
